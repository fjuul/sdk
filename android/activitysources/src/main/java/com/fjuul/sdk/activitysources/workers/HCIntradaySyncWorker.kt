...
class HCIntradaySyncWorker(context: Context, workerParams: WorkerParameters) :
    ...
    override fun doWork(): Result {
        ...
        val dailySyncOptions = buildDailySyncOptions()
        hcSource.syncDaily(dailySyncOptions) { result ->
            if (result.isError && result.error is Exception) {
                taskCompletionSource.trySetException(result.error as Exception)
                return@syncDaily
            }
    ...
    private fun buildIntradaySyncOptions(): HealthConnectSyncOptions {
        ...
        val rawProfileMetrics = inputData.getStringArray(KEY_HC_INTRADAY_METRICS) ?: emptyArray()
    ...
    private fun buildDailySyncOptions(): HealthConnectSyncOptions {
        val metrics = mutableSetOf<FitnessMetricsType>()
        val rawProfileMetrics = inputData.getStringArray(KEY_HC_DAILY_METRICS) ?: emptyArray()
        for (rawProfileMetric in rawProfileMetrics) {
            try {
                val metric = FitnessMetricsType.valueOf(rawProfileMetric)
                metrics.add(metric)
