...
//
    ...
    private func fetchWorkoutsUpdates(completion: @escaping (_ data: HKRequestData?, _ newAnchor: HKQueryAnchor?) -> Void) {
        ...
        let anchorDate = self.getAnchorBySampleType(sampleType: HKObjectType.workoutType())
        let query = HKAnchoredObjectQuery(type: HKObjectType.workoutType(),
            ...
            cycleDispatchGroup.notify(queue: .global(qos: .userInitiated)) {
                completion(HKRequestData(workouts: workouts), nil)
            }
        }
        HealthKitManager.healthStore.execute(query)
    }

    private func fetchWorkoutSmaples(sampleType: HKQuantityType, workout: HKWorkout, completion: @escaping (Result<[HKQuantitySample], Error>) -> Void) {
    ...
    private func convertWorkoutQuantitySamples(samples: [HKQuantitySample], unit: HKUnit) -> [WorkoutSampleData] {
        var workoutSamples: [WorkoutSampleData] = []
        for sample in samples {
            let workoutSample = WorkoutSampleData(value: sample.quantity.doubleValue(for: unit), startDate: sample.startDate, endDate: sample.endDate, uuid: sample.uuid)
